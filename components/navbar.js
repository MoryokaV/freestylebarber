...
const MenuButton = ({ action, active }) => (
  <button
    ...
    className="transition-all inline-flex justify-center rounded-md border border-zinc-700 p-2 text-sm sm:hidden shadow-sm hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-efGreen appearance-none"
    id="menu-button"
...
const Menu = ({ active }) => (
  <div
    className={`${
      active ? 'opacity-1 scale-100 pointer-events-auto' : 'opacity-0 scale-50 pointer-events-none'
    } transition absolute right-0 z-20 mt-2 w-56 origin-top-right rounded-md bg-bodyBgAccent shadow-lg border border-zinc-700 focus:outline-none`}
    role="menu"
  ...
  >
    <div className="py-1" role="none">
      <div className="px-3 py-2 uppercase font-bold text-xs">Secțiuni</div>
